...
public class BatchRegController {
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_SUPERUSER')")
    public ResponseEntity<ApiResponse<List<BatchRegResponseDto>>> getByBatchEncWithFilters(
            @PathVariable Long batchId,
            @RequestParam(required = false) String serial,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate start,
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate end
    ) {
        String sanitizedSerial = (serial != null && serial.isBlank()) ? null : serial;
        ApiResponse<List<BatchRegResponseDto>> response = batchRegService.getByBatchEncWithFilters(batchId, sanitizedSerial, start, end);
        return ResponseEntity.status(HttpStatus.valueOf(response.meta().status())).body(response);
    }
